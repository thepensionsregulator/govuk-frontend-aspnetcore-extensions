...
16. To use validation, in `Startup.cs` add the following to the `Configure` method:
    ...
    // Note: two extra services are being injected
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IOptions<MvcOptions> mvcOptions, IUmbracoContextAccessor umbracoContextAccessor)
    {
        ...
        // Repeat this line for each page requiring validation support.
        // Replace Home with the model generated by Umbraco Models Builder for your document type.
        // Replace Home.Blocks with the property which uses the 'GOV.UK Block List' data type.
        mvcOptions.Value.ModelMetadataDetailsProviders.Add(new UmbracoBlockListValidationMetadataProvider(umbracoContextAccessor, nameof(Home), nameof(Home.Blocks)));
    }
    ```
