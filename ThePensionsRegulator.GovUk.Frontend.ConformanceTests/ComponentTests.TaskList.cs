...
using Xunit;
using GovUkHtml = ThePensionsRegulator.GovUk.Frontend.HtmlGeneration;
...
namespace ThePensionsRegulator.GovUk.Frontend.ConformanceTests
{
    public partial class ComponentTests
    {
        ...
        private static HtmlGeneration.TaskStatus BuildStatus(ThePensionsRegulator.GovUk.Frontend.ConformanceTests.OptionsJson.TaskListTask item)
        {
            var status = new GovUkHtml.TaskStatus();
            ...
            if (Enum.TryParse(typeof(TaskListTaskStatus), status.Content?.ToHtmlString(HtmlEncoder.Default), out var parsedStatus))
            {
