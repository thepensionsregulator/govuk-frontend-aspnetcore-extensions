...
## Test that content is overridden  
Properties in Umbraco can be overridden with values generated by your code. This is typically done when populating a summary list with data the user has entered or replacing a token in a string with a value. You should write tests to prove this happens correctly. 

### Setup the blocks
Any instance of `UmbracoContentFactory.CreateContent()` should be wrapped in a `OverridablePublishedElement`. For example:

```csharp
var summaryListActionBlock =  UmbracoBlockListFactory.CreateOverridableBlock(new OverridablePublishedElement(
    UmbracoContentFactory.CreateContent<IOverridablePublishedElement>(ElementTypeAliases.SummaryListAction)
            .SetupUmbracoTextboxPropertyValue(PropertyAliases.SummaryListActionLinkText, "Change")
            .SetupUmbracoMultiUrlPickerPropertyValue(PropertyAliases.SummaryListActionLink, new Link { Url = "/change-url" })
            .Object)),

```
### Assert block has property overridden
Once you have called the code being tested. You can assert that the block has properties containing the new values. For example:
```csharp
var summaryListActionLink = summaryListActionBlock.Content.Value<Link>(PropertyAliases.SummaryListActionLink);
Assert.That(summaryListActionLink.Url, Is.EqualTo("/my-new-url"))
