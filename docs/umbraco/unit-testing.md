...
## Testing content is overridden  
Blocks in Umbraco can be overridden with blocks generated by your code. This is typically done when populating a summary list with data the user has entered or replacing a token in a string with a value. You should write tests to prove this happens correctly. 
...
```csharp
var summaryListActionLink = summaryListActionBlock.Content.Value<string>(PropertyAliases.SummaryListActionLink);
