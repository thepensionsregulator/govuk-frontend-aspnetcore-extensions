...
{
    public class TprGlobalNavigationService(IPublishedValueFallback _publishedValueFallback) : ITprGlobalNavigationService
    {
        public IList<TprHeaderMenuItem> GetMenuItems(IPublishedContent settingsNode, TprHeaderMenuViewModel tprHeaderMenuViewModel)
        {
            ...
            var headerMenuBlockList = settingsNode?.Value<OverridableBlockListModel>(_publishedValueFallback, tprHeaderMenuViewModel.MenuBlockListAlias);
            ...
            foreach (var item in headerMenuBlockList)
            {
                ...
                var linkUrl = item?.Content.Value<Link>(_publishedValueFallback, tprHeaderMenuViewModel.LinkUrlAlias);
                var linkTextValue = item?.Content.Value<string>(_publishedValueFallback, tprHeaderMenuViewModel.LinkTextAlias);
                ...
                var headerMenuChildItems = item?.Content.Value<OverridableBlockListModel>(_publishedValueFallback, tprHeaderMenuViewModel.MenuItemsChildAlias);
                if (headerMenuChildItems != null)
                {
                    foreach (var child in headerMenuChildItems)
                    {                      
                        var childLinkUrl = child?.Content.Value<Link>(_publishedValueFallback, tprHeaderMenuViewModel.LinkUrlAlias);
                        var childLinkTextValue = child?.Content.Value<string>(_publishedValueFallback, tprHeaderMenuViewModel.LinkTextAlias);
