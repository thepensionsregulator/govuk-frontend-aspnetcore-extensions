...
namespace ThePensionsRegulator.Umbraco.Core.Tests.Blocks
{
    ...
    public class OverridableBlockModelExtensionsTests
    {
        ...
        public void FindBlock_on_OverridableBlockListModel_returns_overridden_value_without_casting()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.Equal(OVERRIDDEN_VALUE, result.Content.Value<string>(Mock.Of<IPublishedValueFallback>(), EXAMPLE_TEXTBOX_PROPERTY_ALIAS));
        }
        ...
        public void FindBlock_on_OverridableBlockListModel_matches_on_overridden_value()
        {
            ...
            var result = blockList.FindBlock(x => x.Content.Value<string>(Mock.Of<IPublishedValueFallback>(), EXAMPLE_TEXTBOX_PROPERTY_ALIAS) == OVERRIDDEN_VALUE);
        ...
        private static (OverridableBlockListModel BlockList, IList<OverridableBlockListItem> BlocksToMatch) CreateOverridableChildBlockListWithMultipleMatchingBlocks()
        {
            ...
            var grandChildBlockList = new OverridableBlockListModel(Mock.Of<IPublishedValueFallback>(), Mock.Of<IOverridablePublishedElementFactoryAccessor>(), [matchingBlock1, matchingBlock2], null);
