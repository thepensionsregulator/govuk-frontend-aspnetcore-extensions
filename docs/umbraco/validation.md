...
- a property otherwise used by standard validators such as `ThePensionsRegulator.GovUk.Frontend.Umbraco.PropertyAliases.ErrorMessageRange`, so long as you don't need a different message for the standard validator on the same field
- a custom property set up by this package for your use such as `ThePensionsRegulator.GovUk.Frontend.Umbraco.PropertyAliases.ErrorMessageCustom1`
