...
1. Ensure you build configuration is set to 'Debug', and build the project(s) you want to test. This will generate an updated NuGet package in the `bin\Debug` folder of each project.
2. Rename the `*.nupkg` file to `*.zip` or associate `*.nupkg` files with a program that can open ZIP files.
...
5. Ensure your build configuration is set to 'Debug', and build the project(s) you want to test. This will generate an updated NuGet package in the `bin\Debug` folder of each project.
6. Return to the consuming application and open the 'Manage NuGet Packages' dialog. Go to the 'Installed' tab and ensure that the 'Include prerelease' box is ticked, and the package source is set to 'All'. Enter 'ThePensionsRegulator' in the search box. Your test version should appear as an available upgrade.
