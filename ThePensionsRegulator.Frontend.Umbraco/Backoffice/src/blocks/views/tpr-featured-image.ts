...
@customElement('tpr-featured-image')
export class TprFeaturedImageView extends UmbElementMixin(LitElement) implements UmbBlockEditorCustomViewElement {

    @property({ attribute: false })
    content?: ITprFeaturedImageContent;

    @property({ attribute: false })
    settings?: ITprFeaturedImageSettings;
    ...
    override render() {
        ...
        if (this.content?.blocks?.contentData?.length === 1) { blocksText = "1 block." }
            ...
            <a href="${this.config?.editContentPath ?? ''}" class="backoffice-block-view">
                <div class="tpr-featured-image${this.settings?.horizontal ? ' tpr-featured-image--horizontal' : null}">
                    <div class="tpr-featured-image_thumbnail">
                        ${displayImage ? html`<img src="${this.#imageUrl?.url}" alt="${altText}" class="${this.settings?.cssClasses}" />` : null}
                    ...
                    <div class="tpr-box tpr-featured-image_info">
...
export default TprFeaturedImageView;
